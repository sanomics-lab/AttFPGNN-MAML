...
class MoleculeMetaMolRelationFeatures(FSMolBatch):
    fingerprints: np.ndarray  # [num_samples, FP_DIM]
    descriptors: np.ndarray  # [num_samples, DESC_DIM]
    mixed_fps: np.ndarray      # [num_samples, MIX_FP_DIM]


@dataclass(frozen=True)
class MetaMolRelationBatch:
    support_features: MoleculeMetaMolRelationFeatures
    support_labels: np.ndarray
    # support_numeric_labels: np.ndarray
    query_features: MoleculeMetaMolRelationFeatures
...
@dataclass(frozen=True)
class FeaturisedMetaMolRelationTaskSample:
    task_name: str
...
def batcher_add_sample_fn(batch_data: Dict[str, Any], sample_id: int, sample: MoleculeDatapoint):
    ...
    batch_data["mixed_fps"].append(all_fps[smi2id[sample.smiles]])


def batcher_finalizer_fn(batch_data: Dict[str, Any]) -> Tuple[MoleculeMetaMolRelationFeatures, np.ndarray]:
    plain_batch = fsmol_batch_finalizer(batch_data)
    return (
        MoleculeMetaMolRelationFeatures(
            fingerprints=np.stack(batch_data["fingerprints"], axis=0),
            descriptors=np.stack(batch_data["descriptors"], axis=0),
            mixed_fps=np.stack(batch_data["mixed_fps"], axis=0),
...
def task_sample_to_metamolrelation_task_sample(
    task_sample: FSMolTaskSample, batcher: FSMolBatcher[MoleculeMetaMolRelationFeatures, np.ndarray]
) -> FeaturisedMetaMolRelationTaskSample:
    ...
    return FeaturisedMetaMolRelationTaskSample(
...
def get_metamolrelation_batcher(
    max_num_graphs: Optional[int] = None,
    max_num_nodes: Optional[int] = None,
    max_num_edges: Optional[int] = None,
) -> FSMolBatcher[MoleculeMetaMolRelationFeatures, np.ndarray]:
    return FSMolBatcher(
        max_num_graphs,
        max_num_nodes,
        max_num_edges,
...
def get_metamolrelation_task_sample_iterable(
    ...
    def path_to_batches_pipeline(paths: List[RichPath], idx: int):
        ...
        for _ in range(num_samples):
            ...
            yield task_sample_to_metamolrelation_task_sample(task_sample, batcher)
