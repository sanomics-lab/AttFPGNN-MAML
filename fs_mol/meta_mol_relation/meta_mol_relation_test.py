...
def parse_command_line():
    parser = argparse.ArgumentParser(
        description="Test a ProtoMolRelation model on molecules.",
...
def test(
    ...
    return evaluate_metamolrelation_model(
        model,
        dataset,
...
def main():
    ...
    out_dir, dataset = set_up_test_run("MetaMolRelation", args, torch=True)
    ...
    model_weights_file = resolve_starting_model_file(
        ...
        model_cls=MetaMolRelationModelTrainer,
        out_dir=out_dir,
        use_fresh_param_init=args.use_fresh_param_init,
        device=device,
    )

    model = MetaMolRelationModelTrainer.build_from_model_file(
        model_weights_file,
        device=device,
    ).to(device)
