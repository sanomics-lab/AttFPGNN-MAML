...
if __name__ == "__main__":
    ...
    with open("ADKF-IFT/datasets/fsmol-0.1.json") as fr:
        fsmol_filenames = json.load(fr)

    base_dir = "ADKF-IFT/datasets/FS-Mol-kaggle/"

    def run_fps(in_smi):
        return in_smi, get_mixed_fps(Chem.MolFromSmiles(in_smi))

    all_smis, all_fps = [], []
    with multiprocessing.Pool(8) as pool:
        # for folder, filenames in fsmol_filenames.items():
        for folder in ["train", "valid", "test"]:
            results = []
            filenames = fsmol_filenames[folder]
            for filename in tqdm.tqdm(filenames):
                fullpath = os.path.join(base_dir, folder, f"{filename}.jsonl")
                df = pd.read_json(fullpath, lines=True)
                for smi in df["SMILES"]:
                    result = pool.apply_async(run_fps, args=(smi, ))
                    results.append(result)

            for result in tqdm.tqdm(results):
                output = result.get()
                all_smis.append(output[0])
                all_fps.append(np.array(output[1]).astype(np.float32))
