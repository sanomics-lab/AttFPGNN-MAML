...
if __name__ == "__main__":
    ...
    TASK_NAMES = ["muv", "sider", "tox21", "toxcast"]

    def run_fps(in_smi):
        return in_smi, get_mixed_fps(Chem.MolFromSmiles(in_smi))

    all_smis, all_fps = set(), []

    for TASK_NAME in TASK_NAMES:
        print(f'TASK_NAME: {TASK_NAME}')
        for folder in os.listdir(os.path.join(TASK_NAME, "new")):
            if not folder.isdigit():
                continue
            filename = os.path.join(TASK_NAME, "new", folder, "processed", "smiles.csv")
